-- LearnSum MVP — Seeker profile fields (bio + phone) + onboarding profile write
-- Migration: 0022_profile_bio_phone.sql
-- Note: local record of objects applied manually via the Supabase SQL editor.
--       Run AFTER 0021. Paste and run the WHOLE file.
--
-- The Expo app added a seeker "About you" screen (SeekerAbout) that collects the
-- student/parent's name, gender, photo, BIO and PHONE (+ a student education
-- level). Name + gender already persisted (POST /api/onboarding → normalizeGender
-- + this RPC); the student school_level edit + gender already worked via
-- PATCH /api/profiles/me. The only missing pieces are bio / phone (no column yet)
-- and avatar_url at onboarding (the RPC never wrote it).
--
-- What changes:
--   • profiles                : + bio text, + phone text
--   • complete_onboarding RPC : the shared-profile UPDATE now also sets
--                               avatar_url, bio, phone. The body below is the
--                               CURRENT (0021) definition verbatim apart from
--                               those three lines — do NOT base this on 0009, which
--                               predates the tutor levels/education/format/district
--                               and seeker district-array writes.
--
-- (The matching route changes — POST /api/onboarding reads avatar_url/bio/phone
-- from the `profile` block, and PATCH /api/profiles/me accepts bio/phone — live in
-- the app code, not this migration.)
...
BEGIN
  ...
  -- Optional shared profile fields (now incl. avatar_url / bio / phone)
