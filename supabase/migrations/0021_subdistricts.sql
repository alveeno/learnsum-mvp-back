-- LearnSum MVP — Subdistrict locations
-- Migration: 0021_subdistricts.sql
-- Note: local record of objects applied manually via the Supabase SQL editor.
--       Run AFTER 0020. Paste and run the WHOLE file.
--
-- The Expo app replaced the 18 coarse `hk_district` choices with ~88 SUBDISTRICTS
-- (region → district → subdistrict, e.g. Wan Chai → Causeway Bay). Locations are
-- now stored as TEXT SLUGS (e.g. "causeway_bay") with the FRONTEND as the source of
-- truth for the list (mirrors how subjects/categories already work), so we move off
-- the `hk_district` enum to text[] rather than growing the enum.
--
-- What changes:
--   • tutor_subcategories.districts : hk_district[]  ->  text[]   (subdistrict slugs)
--   • seeker preferred_districts    : already text[] — now holds subdistrict slugs
--   • complete_onboarding RPC       : tutor per-subject INSERT stores districts as
--                                     text[] (drops the ::hk_district[] cast)
--
-- profiles.district (single hk_district) is left as-is and is no longer the source
-- of a tutor's location — search now matches a tutor's per-subject subdistricts
-- (tutor_subcategories.districts). Any pre-existing enum values in
-- tutor_subcategories.districts simply become their text form (e.g. 'ShaTin') and
-- are harmless (they just won't match the new slugs).

-- 1) Move the per-subject districts column to text[] (subdistrict slugs).
ALTER TABLE tutor_subcategories
  ALTER COLUMN districts TYPE text[] USING districts::text[];

-- 2) complete_onboarding — identical to 0020 except the tutor per-subject INSERT
--    now stores `districts` as text[] (subdistrict slugs), not hk_district[].
