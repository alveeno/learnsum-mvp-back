-- LearnSum MVP — Tutor profile extras (Group A gap fixes)
-- Migration: 0014_tutor_profile_extras.sql
-- Reference: BACKEND_GAP_ANALYSIS.md group A (A1–A5).
-- Note: local record of objects applied manually via the Supabase SQL editor.
--       Run AFTER 0010–0013. Paste and run the WHOLE file.
--
-- Closes the onboarding data-loss gaps where the tutor flow collected info the
-- backend had no home for, so it was silently dropped on save:
--   A1  teaching_levels  — which of the 6 levels a tutor teaches
--   A2  experience       — per-subject "relevant experience" list
--   A3  education / current_studies — multi-school history + current study
--   A4  gender 'lgbt'    — frontend offers LGBTQ+; add it to the enum
--   A5  (names)          — handled in /api/onboarding (combine first/last → full_name)
-- complete_onboarding() is updated to persist A1–A3.

-- ---------------------------------------------------------------------------
-- A4. Add 'lgbt' to the gender enum. The frontend's "LGBTQ+" maps to it; "N.A."
--     maps to prefer_not_to_say (mapping done in /api/onboarding, not here).
--     PG 15 allows ADD VALUE as long as the value is not USED in the same
--     transaction — defining complete_onboarding below only references the type,
--     it does not use the literal 'lgbt', so this is safe.
-- ---------------------------------------------------------------------------
ALTER TYPE gender_type ADD VALUE IF NOT EXISTS 'lgbt';

-- ---------------------------------------------------------------------------
-- A1 / A3. Tutor profile columns.
--   teaching_levels  — array of the 6-value school_level enum (multi-select).
--   education        — jsonb history per level, e.g.
--                      { "secondary": [{ "institution","qualification","score" }], ... }
--   current_studies  — jsonb array, e.g. [{ "institution","programme" }]
-- ---------------------------------------------------------------------------
ALTER TABLE tutor_profiles
  ADD COLUMN IF NOT EXISTS teaching_levels school_level[],
  ADD COLUMN IF NOT EXISTS education       jsonb,
  ADD COLUMN IF NOT EXISTS current_studies jsonb;

-- ---------------------------------------------------------------------------
-- A2. Per-subject "relevant experience" list (jsonb array of entries collected
--     on the tutor "Strengths & Details" screen).
-- ---------------------------------------------------------------------------
ALTER TABLE tutor_subcategories
  ADD COLUMN IF NOT EXISTS experience jsonb;

-- ---------------------------------------------------------------------------
-- complete_onboarding — identical to 0010 except the TUTOR branch now also
-- persists teaching_levels, education, current_studies and per-subject
-- experience. The student/parent branches are unchanged.
-- ---------------------------------------------------------------------------
