-- LearnSum MVP — Per-subject teaching levels
-- Migration: 0020_tutor_subcategory_levels.sql
-- Note: local record of objects applied manually via the Supabase SQL editor.
--       Run AFTER 0019. Paste and run the WHOLE file.
--
-- The Expo app moved teaching-level selection (which age groups a tutor teaches)
-- from a single tutor-level screen INTO the per-subject "Strengths & Details"
-- screen, because a tutor can teach different age groups for different subjects
-- (e.g. Maths P1–S3, Piano all ages). The tutor-level `teaching_levels` stays (it
-- holds the union, for a quick "teaches these levels" display); this adds the
-- per-subject `levels` so the granularity is actually stored.
--
-- `levels` uses the same school_level enum as the tutor-level teaching_levels and
-- the seeker school_level. Levels are DISPLAY-ONLY (matching doesn't read them).

ALTER TABLE tutor_subcategories
  ADD COLUMN IF NOT EXISTS levels school_level[];

-- ---------------------------------------------------------------------------
-- complete_onboarding — identical to 0016 except the tutor per-subject INSERT
-- now also writes `levels`. All other branches are unchanged.
-- ---------------------------------------------------------------------------
