-- LearnSum MVP — Seeker school history (student_profiles.education)
-- Migration: 0023_seeker_education.sql
-- Note: local record of objects applied manually via the Supabase SQL editor.
--       Run AFTER 0022. Paste and run the WHOLE file.
--
-- The seeker "About you" (SeekerAbout) now lets STUDENTS enter a full per-level
-- school history (the same EducationSection UI as tutors), kept ALONGSIDE the
-- single school_level (which still drives seeker→tutor matching). Store the
-- history as jsonb on student_profiles, mirroring tutor_profiles.education.
--
-- What changes:
--   • student_profiles        : + education jsonb
--   • complete_onboarding RPC : the student INSERT/UPDATE now also writes
--                               education (jsonb). The body below is the CURRENT
--                               (0022) definition verbatim apart from that one
--                               column — do NOT base it on an older migration.
--
-- (Route changes — POST /api/onboarding sends student.education, and
-- PATCH /api/profiles/me accepts it on the student block — live in the app code.)
