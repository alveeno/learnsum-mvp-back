...
- Post likes — **endpoint now live** (`POST`/`DELETE`/`GET /api/posts/[id]/likes`; `liked_by_me` on the posts feed); the like button just needs **frontend** wiring. Comments were dropped by the app (schema stays dormant).
